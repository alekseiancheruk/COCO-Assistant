...
class COCO_Assistant:
    def __init__(self, img_dir=None, ann_dir=None):
        ...
        self.imgfolders = sorted(
            [
                ...
                if (
                    os.path.isdir(os.path.join(self.img_dir, i)) is True
        ...
        self.annfiles = [
            COCO(os.path.join(ann_dir, i)) for i in self.jsonfiles
        ]
    ...
    def merge(self, merge_images=True):
        ...
        if merge_images:
            ...
            im_dirs = [
                os.path.join(self.img_dir, folder)
    ...
    def remove_cat(self, interactive=True, jc=None, rcats=None):
        ...
        if interactive:
            ...
            self.jc = input()
            if self.jc.lower() not in [
                item.lower() for item in self.jsonfiles
            ]:
    ...
    def ann_stats(self, stat, arearng, show_count=False, save=False):
        ...
        if stat == "area":
            stats.pi_area_split(
                self.annfiles, self.names, areaRng=arearng, save=save
            )
        elif stat == "cat":
            stats.cat_count(
                self.annfiles, self.names, show_count=show_count, save=save
            )
    ...
    def visualise(self):
        ...
        if dir_choice.lower() not in [
            item.lower() for item in self.imgfolders
        ]:
